...
@autologgable_output
class RevIntakeIndexer(Subsystem, RpmMechanismIO):
    """
    ...
    def __init__(self, container: 'RobotContainer', can_device_id: int, inverted: bool) -> None:
        ...
        RpmMechanismIO.__init__(self, "IntakeIndexer")
    ...
    def log_motor(self, log):
        log.motor("indexer").field("velocity", self._encoder.getVelocity())
    ...
    def reset(self) -> None:
        self.stop()
        logger.info(f"Intake Indexer: Reset command was called  *** *** *** ***   NOT YET SUPPORTED")
        self._rpm_goal = 0
    ...
    def intake_fuel(self):
        ...
        logger.info(f"Intake Indexer: Intake fuel, currently at: {self._encoder.getPosition()}")
        logger.info(f"Intake Indexer: Intake fuel. RPM goal before command is: {self._rpm_goal}")

        self.set_rpm_goal(self._rpm_goal)

    def expel_fuel(self):
        # Expel any fuel we may have
        logger.info(f"Intake Indexer: Expel fuel, currently at: {self._encoder.getPosition()}")
        logger.info(f"Intake Indexer: Expel fuel. RPM goal before command is: {self._rpm_goal}")
    ...
    def set_rpm_goal(self, goal: revolutions_per_minute) -> None:
        if self._rpm_goal != goal:
            logger.info(f"Intake Indexer: Setting goal RPM to {goal}. currently at: {self._encoder.getPosition()}")
            logger.info(
                f"Intake Indexer: current PID controller setpoint before command is: {self._pid_controller.getSetpoint()}")
    ...
    def stop(self) -> None:
        logger.info(f"Intake Indexer: Stop command was called")
        self._motor.stopMotor()

    def periodic(self) -> None:
        LogTracer.resetOuter("Intake Indexer periodic")

        self.updateInputs(self._inputs)

        Logger.processInputs("Intake Indexer", self._inputs)
        ...
        Logger.recordOutput("Intake/Indexer/goal", self._rpm_goal)
        Logger.recordOutput("Intake/Indexer/closed-loop", self._closed_loop)
    ...
    def dashboard_periodic(self) -> None:
        ...
        """
        SmartDashboard.putNumber("Intake/Indexer/goal", self._rpm_goal)
        SmartDashboard.putBoolean("Intake/Indexer/closed-loop", self._closed_loop)
        SmartDashboard.putNumber("Intake/Indexer/RPM", self._inputs.drive_velocity)
